...
def analizar_uso_datos(df):
    ...
    for cliente_id, grupo in df_agrupado.groupby('cliente_id'):
        uso_total = float(grupo['uso_datos_mb'].sum())
        dias = int(len(grupo))
        promedio_diario = float(uso_total / dias)
        dias_anomalos = int(grupo[grupo['uso_datos_mb'] > 1.2 * promedio_diario]['fecha'].nunique())
        porcentaje_anomalos = float((dias_anomalos / dias) * 100 if dias > 0 else 0)
        
        resumen_mensual[int(cliente_id)] = {
