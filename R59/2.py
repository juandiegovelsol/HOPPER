...
def producción_optimal_dulces(stock, precios):
    ...
    if not isinstance(stock, list) or not isinstance(precios, list):
        print("Error: Las entradas 'stock' y 'precios' deben ser listas.")
        return

    if len(stock) != 4 or len(precios) != 4:
        print("Error: Las listas 'stock' y 'precios' deben tener exactamente 4 elementos.")
        return

    if any(not isinstance(x, (int, float)) for x in stock) or any(not isinstance(x, (int, float)) for x in precios):
        print("Error: Todos los elementos de 'stock' y 'precios' deben ser números.")
        return

    if any(x <= 0 for x in stock) or any(x <= 0 for x in precios):
        print("Error: Todos los elementos de 'stock' y 'precios' deben ser estrictamente positivos.")
        return
    ...
    precios_de_venta = [precios[0], precios[1], precios[2], precios[3]]
    ...
    for i, prod in enumerate(productos):
        obj += precios_de_venta[i] * var_producción_cajas[prod]
    problem += obj 
    ...
    print('\nGano total:', sum(precios_de_venta[i] * resultados_producción[i] for i, prod in enumerate(productos)))

# Ejemplo de uso
producción_optimal_dulces([1, 1, 2 ,3], [10, 5, 15, 10])