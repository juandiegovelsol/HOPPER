...
def cargar_nivel(indice_nivel):
    ...
    for plat_info in nivel:
        x, y, tipo = plat_info
        plataforma = Plataforma(x, y, tipo)  # Se descomponen los parámetros de cada plataforma
        plataformas.add(plataforma)
        if tipo == 'inicio':  # Identifica si la plataforma es de inicio
            x_inicio = plataforma.rect.centerx - JUGADOR_WIDTH // 2  # Establece la posición inicial del jugador
            y_inicio = plataforma.rect.top - JUGADOR_HEIGHT

    if jugador is None:
        jugador = Jugador(x_inicio, y_inicio)  # Crea al jugador
    else:
        jugador.reiniciar_posicion(x_inicio, y_inicio)  # Reasigna la posición del jugador
...
while True:
    ...
    if estado_juego == "jugando":
        ...
        colisiones_meta = pygame.sprite.spritecollide(jugador, plataformas, False)
        for plat in colisiones_meta:
