function procesarInfoAlumnos(jsonData) {
  ...
  for (const alumno of alumnos) {
    ...
    const mediaGeneral = calcularMediaGeneral(alumno.cursos);
    resultadosFinales.push({
      id: alumno.id,
      nombre: alumno.nombre,
      semestre: alumno.semestre,
      promedioSemestre: mediaSemestre,
      promedioGeneral: mediaGeneral,
    });
  }
...
// Nueva función para calcular el promedio de todas las notas de los cursos de cada estudiante
function calcularMediaGeneral(cursos) {
  const sumaNotasTotales = cursos.reduce((acum, curso) => acum + curso.nota, 0);
  return sumaNotasTotales / cursos.length;
}
